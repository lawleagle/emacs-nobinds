...
(defvar query-replace-map
  (let ((map (make-sparse-keymap)))
    (add-keybind map " " 'act)
    (add-keybind map "\d" 'skip)
    (add-keybind map [delete] 'skip)
    (add-keybind map [backspace] 'skip)
    (add-keybind map "y" 'act)
    (add-keybind map "n" 'skip)
    (add-keybind map "Y" 'act)
    (add-keybind map "N" 'skip)
    (add-keybind map "e" 'edit-replacement)
    (add-keybind map "E" 'edit-replacement)
    (add-keybind map "," 'act-and-show)
    (add-keybind map "q" 'exit)
    (add-keybind map "\r" 'exit)
    (add-keybind map [return] 'exit)
    (add-keybind map "." 'act-and-exit)
    (add-keybind map "\C-r" 'edit)
    (add-keybind map "\C-w" 'delete-and-edit)
    (add-keybind map "\C-l" 'recenter)
    (add-keybind map "!" 'automatic)
    (add-keybind map "^" 'backup)
    (add-keybind map "u" 'undo)
    (add-keybind map "U" 'undo-all)
    (add-keybind map "\C-h" 'help)
    (add-keybind map [f1] 'help)
    (add-keybind map [help] 'help)
    (add-keybind map "?" 'help)
    (add-keybind map "\C-g" 'quit)
    (add-keybind map "\C-]" 'quit)
    (add-keybind map "\C-v" 'scroll-up)
    (add-keybind map "\M-v" 'scroll-down)
    (add-keybind map [next] 'scroll-up)
    (add-keybind map [prior] 'scroll-down)
    (add-keybind map [?\C-\M-v] 'scroll-other-window)
    (add-keybind map [M-next] 'scroll-other-window)
    (add-keybind map [?\C-\M-\S-v] 'scroll-other-window-down)
    (add-keybind map [M-prior] 'scroll-other-window-down)
    ;; Binding ESC would prohibit the M-v binding.  Instead, callers
    ;; should check for ESC specially.
    ;; (define-key map "\e" 'exit-prefix)
    (add-keybind map [escape] 'exit-prefix)
    map)
...
(defvar multi-query-replace-map
  (let ((map (make-sparse-keymap)))
    ...
    (add-keybind map "Y" 'automatic-all)
    (add-keybind map "N" 'exit-current)
    map)
