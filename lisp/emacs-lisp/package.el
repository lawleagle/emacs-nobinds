...
(defvar package-menu-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (add-keybind map "m" 'package-menu-describe-package)
    (add-keybind map "u" 'package-menu-mark-unmark)
    (add-keybind map "\177" 'package-menu-backup-unmark)
    (add-keybind map "d" 'package-menu-mark-delete)
    (add-keybind map "i" 'package-menu-mark-install)
    (add-keybind map "U" 'package-menu-mark-upgrades)
    (add-keybind map "r" 'package-menu-refresh)
    (add-keybind map "f" 'package-menu-filter)
    (add-keybind map "~" 'package-menu-mark-obsolete-for-deletion)
    (add-keybind map "x" 'package-menu-execute)
    (add-keybind map "h" 'package-menu-quick-help)
    (add-keybind map "H" #'package-menu-hide-package)
    (add-keybind map "?" 'package-menu-describe-package)
    (add-keybind map "(" #'package-menu-toggle-hiding)
    map)
...
shown."
  ...
  (setf (car (aref tabulated-list-format 0))
        (if keywords
          ...
          "Package"))
  (if keywords
      (add-keybind package-menu-mode-map "q" 'package-show-package-list)
    (add-keybind package-menu-mode-map "q" 'quit-window))
  (tabulated-list-init-header)
